...
- The Windows UI groups steering-load and speed-stability controls under `Steering & Stability`. The calculator keeps the layers separate: load resistance can add spring, damper, and friction, while speed stability adds damper for speed/rate damping and anti-oscillation.
...
- `effectStatus.json` keeps separate `steeringLoad` and `speedStability` fields for compatibility, while the Windows overlay presents their lamps in one `Steering & Stability` group. `loadSlopeImplement` remains an aggregate indicator for load, Motion, hill standstill, side slope, and implement-bias activity. The Windows overlay has separate lamps for Motion, Hill Standstill Load, Side Slope Bias, and Implement Bias.
