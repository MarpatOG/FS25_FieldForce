...
public sealed class TelemetryReceiverServiceTests
{
    private const string ValidPacket = """
        {
          "protocol": { "name": "FIELDFORCE_TELEMETRY", "version": "1.2.0" },
    ...
    public void Selected_folder_is_resolved_to_telemetry_file_path()
    {
        ...
        var telemetryFolder = Path.Combine(modSettings, "FS25_FieldForceTelemetry");
        var legacyTelemetryFolder = Path.Combine(modSettings, "FS25_RealFfbTelemetry");

        Assert.Equal(
            Path.Combine(documents, "My Games", "FarmingSimulator2025", "modSettings", "FS25_FieldForceTelemetry", "telemetry.json"),
            TelemetryReceiverService.ResolveTelemetryFilePathFromSelectedFolder(documents));
        Assert.Equal(
            Path.Combine(myGames, "FarmingSimulator2025", "modSettings", "FS25_FieldForceTelemetry", "telemetry.json"),
            TelemetryReceiverService.ResolveTelemetryFilePathFromSelectedFolder(myGames));
        Assert.Equal(
            Path.Combine(gameProfile, "modSettings", "FS25_FieldForceTelemetry", "telemetry.json"),
            TelemetryReceiverService.ResolveTelemetryFilePathFromSelectedFolder(gameProfile));
        Assert.Equal(
            Path.Combine(modSettings, "FS25_FieldForceTelemetry", "telemetry.json"),
        ...
        Assert.Equal(
            Path.Combine(legacyTelemetryFolder, "telemetry.json"),
            TelemetryReceiverService.ResolveTelemetryFilePathFromSelectedFolder(legacyTelemetryFolder));
    }

    [Fact]
    public void Legacy_file_path_uses_old_mod_settings_folder()
    {
        var userProfile = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
        if (string.IsNullOrWhiteSpace(userProfile))
        {
        ...
        Assert.Equal(expected, TelemetryReceiverService.GetLegacyTelemetryFilePath());
    }
    ...
    public void Telemetry_contract_accepts_v1_2_engine_drivetrain_fields()
    {
        var json = """
            {
              ...
              "engine": { "isRunning": false, "started": false, "state": "starting", "isStarting": true, "startDurationMs": 1200, "startRemainingMs": 800, "rpm": 900, "rpm01": 0.25, "minRpm": 500, "maxRpm": 2400, "load01": 0.6, "torque": 120, "maxTorque": 400, "motorType": "diesel", "powertrainType": "hybrid", "energySources": ["diesel", "electricCharge"] },
        ...
        Assert.Equal("hybrid", packet.PowertrainType);
        Assert.Equal(["diesel", "electricCharge"], packet.EnergySources);
    ...
    public void Telemetry_contract_accepts_v1_3_tire_surface_wheel_fields()
    {
        var json = """
            {
              "protocol": { "name": "FIELDFORCE_TELEMETRY", "version": "1.3.0" },
    ...
    public void Telemetry_contract_accepts_position_spike_filtered_packet()
    {
        var json = """
            {
              "protocol": { "name": "FIELDFORCE_TELEMETRY", "version": "1.3.0" },
              "frame": { "sequence": 44, "dtMs": 16, "telemetryRateHz": 60, "timestampMs": 123504, "isDuplicate": false, "isInterpolated": false },
              "game": { "state": "mission" },
              "player": { "isInVehicle": true },
              "vehicle": { "name": "Tractor", "type": "tractor", "category": "TractorWheeled", "massT": 6.2, "totalMassT": 8.8 },
              "controls": { "throttle": 0.42, "brake": 0.0, "clutch": 0.0 },
              "motion": {
                "speedMps": 3.6666666667,
                "speedKmh": 13.2,
                "pitchDeg": 0.1,
                "rollDeg": 0.0,
                "yawRateRadPerSec": null,
              ...
              "steering": { "angle": 0.02, "rate": 0.0 },
              "engine": { "isRunning": true, "started": true, "rpm": 1250 },
              "transmission": { "gear": 2 },
              "wheels": [],
              "suspension": { "impulse": null, "verticalImpactImpulse": null, "landingImpulse": null, "leftImpulse": null, "rightImpulse": null },
              "surface": { "isOnField": false, "type": "asphalt", "attribute": 3 },
              "environment": { "groundWetness": 0.0, "rainScale": 0.0 },
              "attachments": [],
              "collisions": { "collisionImpulse": null, "longitudinalJerkImpulse": null },
              "diagnostics": { "payloadBytes": 1800, "buildTimeMs": 0.4, "warnings": [] }
            }
        ...
        packet!.ValidateContract();
        Assert.True(packet.IsProtocolValid);
        Assert.Equal(13.2, packet.SpeedKmh);
        Assert.Equal(3.6666666667, packet.Motion?.SpeedMps);
        Assert.Null(packet.LocalAccelerationZ);
        Assert.Null(packet.CollisionImpulse);
        Assert.Null(packet.LongitudinalJerkImpulse);
    }
    ...
    [Fact]
    public void Telemetry_contract_accepts_legacy_protocol_name()
    {
        var packet = JsonSerializer.Deserialize<TelemetryPacketV1>(
            ValidPacket.Replace("FIELDFORCE_TELEMETRY", "FS25_REAL_FFB_TELEMETRY", StringComparison.Ordinal));
    ...
    public async Task Rejects_wrong_protocol_name()
    {
        ...
        await SendUdpAsync(port, ValidPacket.Replace("FIELDFORCE_TELEMETRY", "OTHER_PROTOCOL", StringComparison.Ordinal));
