...
public sealed class MainWindowViewModelTests
{
    ...
    [Fact]
    public void Reload_FieldForce_restarts_runtime_without_disabling_gameplay_ffb()
    {
        var directory = Path.Combine(Path.GetTempPath(), "FieldForce.Tests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(directory);
        var configPath = Path.Combine(directory, "config.json");
        ...
        viewModel.ReloadFieldForceCommand.Execute(null);
    ...
    [Fact]
    public void Reload_FieldForce_handles_device_scan_failure_without_crashing()
    {
        var directory = Path.Combine(Path.GetTempPath(), "FieldForce.Tests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(directory);
        var configPath = Path.Combine(directory, "config.json");
        ...
        var exception = Record.Exception(() => viewModel.ReloadFieldForceCommand.Execute(null));
