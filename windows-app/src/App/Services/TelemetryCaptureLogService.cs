...
public sealed class TelemetryCaptureLogService : IDisposable
{
    ...
    private static string CreateCsvLine(DateTimeOffset recordedAt, TelemetryReceiverState state)
    {
        ...
        var values = new string?[]
        {
            ...
            Format(packet?.RoadSlopeLongitudinalDeg),
            Format(packet?.RoadSlopeLateralDeg),
            packet?.RoadSlopeSource,
            Format(packet?.RoadSlopeConfidence),
            ...
            Format(packet?.LeftBottomOutImpulse),
            Format(packet?.RightBottomOutImpulse),
            Format(packet?.BottomOutImpulse),
            Format(packet?.MaxSuspensionVelocity),
            Format(packet?.MaxTireLoad),
            packet?.CompressionRatioAvailable == true ? "true" : "false",
    ...
    [
        ...
        "leftBottomOutImpulse",
        "rightBottomOutImpulse",
        "bottomOutImpulse",
        "maxSuspensionVelocity",
        "maxTireLoad",
        "compressionRatioAvailable",
        "landingImpulse",
        "collisionImpulse",
